...
if(highlighted_beard + scroller >= scr_number_beards_unlocked()){
	...
	draw_sprite(spr_paper_bag, 0, GRID_SIZE * 11, GRID_SIZE * 13.5);
	draw_sprite(spr_paper_bag, 0, GRID_SIZE * 7, GRID_SIZE * 13.);
...
if(box.open && !confirm){
	for(i = 0; i < menu_max_list; i++){
		...
		if(ds_grid_get(global.all_beard_properties, scroller + i, BRD_UNLOCKED)){
			...
			draw_text(GRID_SIZE * 28, GRID_SIZE * 6.5 + (i * GRID_SIZE), string(ds_grid_get(global.all_beard_properties, scroller + i, BRD_SCORE_TO_UNLOCK)) + " points to unlock")
		}
...
if(highlighted_beard + scroller < ds_grid_width(global.all_beard_properties) - 1 && ds_grid_width(global.all_beard_properties) > menu_max_list){
	...
	draw_sprite(spr_scroll_arrow, 0, 100, 100);
...
if(scroller > 0){
	...
	draw_sprite_ext(spr_scroll_arrow, 0, 200, 200, 1, -1, 0, c_white, 1);
